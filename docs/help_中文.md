...
BlogBuddy 是一个强大的 VS Code 扩展，通过 AI 功能增强你的写作工作流。无论你是在撰写技术文档、博客文章，还是任何 Markdown 内容，BlogBuddy 都能在编辑器中提供智能的文本处理能力。

## 🚀 快速开始

1. **安装并配置**：在 VS Code 设置中配置你的 API 密钥
2. **打开 Markdown 文件**：BlogBuddy 在 `.md` 文件中效果最佳
...
在文本中直接嵌入特殊标签以进行即时 AI 处理。

### 3. 文档统计 (Ctrl+Shift+D)

在 Markdown 文件的状态栏中实时显示字数统计。

---

## 🏷️ BB 命令系统

BB 命令系统允许你使用特殊标签在文本中直接嵌入 AI 指令。你可以将命令放在与内容同一行或单独的行中。将光标放在文本块的任意位置（或选择特定文本），然后按 `Cmd+B Cmd+B`（Mac）或 `Ctrl+B Ctrl+B`（Win/Linux）来执行。

**工作原理：**

- **文本块模式**：当 BB 命令与其他内容处于同一文本块（由空行分隔）时，仅处理该文本块
- **全文模式**：当 BB 命令单独占据一行且上下有空行（文本块中无其他内容）时，处理整个文档
...
**用途**：在保持原意的前提下扩展并详述现有内容  
**用法**：将命令放在你想扩展的内容所在的同一文本块中  
**示例**：
...
- **文本块模式**：当 BB 命令与其他内容共享文本块时，仅就该文本块进行原地改进并使用文档上下文
...
**用途**：通过分析文本内容生成 Mermaid 图表并将其转换为可视化表示  
...
- 分析文本内容以理解结构与关系
- 自动选择合适的图表类型（flowchart、sequence、class、state、ER、gantt、pie）
...
**用途**：使用自定义指令执行任何 AI 任务  
**用法**：根据内容选择有两种不同模式

> **⚠️ 重要提示**：这是最强大且最灵活的命令。由于任务的复杂性和开放性，可能需要更高性能的 AI 模型，并可能导致与其他专用命令相比明显更高的令牌使用量和费用。对于无法使用上述专用命令完成的任务，请谨慎使用。
...
- **文本块模式**：当 BB 命令与其他内容共享文本块时，仅在原地处理该文本块
...
- 可能比专用命令消耗更多令牌
- 复杂任务的处理时间可能更长
- 若可能，优先使用专用命令（expd、impv、tslt 等）以提高效率
...
- **智能字数统计**：自动分别检测并统计中文字符和英文单词
- **状态栏显示**：在 VS Code 状态栏中以不干扰的方式显示字数
- **点击切换**：点击状态栏项可快速启用/禁用该功能
- **仅限 Markdown**：统计仅在 Markdown (.md) 文件中显示
- **实时更新**：字数在你输入或编辑内容时自动更新
...
- **Mermaid 输出形式** (`blogbuddy.mermaidCode`)：选择 Mermaid 图表的输出格式
...
1. **掌握空行控制**：空行决定处理范围
   ...
   - **被空行隔离**：处理整个文档
2. **选择你的模式**：
   - 对于本地编辑：将命令与内容放在同一文本块（中间无空行）
   - 对于全文处理：在命令上方和下方使用空行将其隔离
3. **上下文很重要**：BB 命令会读取整个文档以获得更好上下文
4. **明确指示**：在命令消息中提供清晰的指令
5. **文件类型**：在 Markdown (.md) 文件中效果最佳
6. **备份重要工作**：BB 会为文档级操作创建新文件

### 工作流集成

1. **先起草**：先写初稿，然后用 BB 命令进行增强
2. **迭代改进**：使用多个命令逐步提升内容
3. **审阅输出**：在定稿前始终审核 AI 生成内容
4. **组合功能**：可使用扩展 → 改进 → 翻译 的工作流
...
| View Usage Stats | 菜单 → 使用统计 | N/A |
| Open Help | 菜单 → 帮助信息 | N/A |
...
- 解决方案：在 VS Code 设置的 BlogBuddy 部分设置 API Key
...
BlogBuddy 正在积极开发并定期更新。新的功能和改进基于用户反馈和 AI 技术进步不断加入。
