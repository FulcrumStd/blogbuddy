...
- **Zen / Read 布局切换** —— 右上角小 `Z` / `R` 按钮，在 **Zen 模式**（居中 860px、专注写作）和 **Read 模式**（铺满视口、适合长篇阅读）之间切换。状态全局保存、跨会话保留；多个 BB 编辑器面板会同步
...
- **默认折叠** —— 面板打开时折叠，点击标题展开。大多数编辑发生在正文，面板平时不占垂直空间，需要时再展开
- 改动随自动保存与正文一起落盘
